...
:items="items">
<template #top>
<div class="top-div"><p class="display-1 white--text">Travel
<span>
<v-tooltip bottom>
...
<template #item.Travel="{item}">
  <td v-if="editable && editindex==items.indexOf(item)">
     <v-text-field class="tx1" v-model="Travel"></v-text-field>
  </td>
  <td id="td1" v-else>{{item.Travel}}</td>
</template>
...
export default{
	...
	headers:[{text:'',value:'Travel',sortable:false},
...
add(){
  ...
  this.editable=true
  this.items.push({Travel:'', Units:'',Total_No_of_Units:'',Total_Unit_value:'',Total_Total_cost:''
...
deleteitem(item){
   ...
   this.$emit('Travel',{subtotal:this.Totalsum,year1sum:this.year1sum})
},
save(e,item,totalcost,year1cost){
  ...
  this.items[index].Travel=this.Travel

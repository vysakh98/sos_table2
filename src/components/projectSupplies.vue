...
:items="items">
<template #top>
<div class="top-div"><p class="display-1 white--text">projectSupplies
<span>
<v-tooltip bottom>
...
<template #item.projectSupplies="{item}">
  <td v-if="editable && editindex==items.indexOf(item)">
     <v-text-field class="tx1" v-model="projectSupplies"></v-text-field>
  </td>
  <td id="td1" v-else>{{item.projectSupplies}}</td>
</template>
...
export default{
	...
	headers:[{text:'',value:'projectSupplies',sortable:false},
...
add(){
  ...
  this.editable=true
  this.items.push({projectSupplies:'', Units:'',Total_No_of_Units:'',Total_Unit_value:'',Total_Total_cost:''
...
deleteitem(item){
   ...
   this.$emit('projectsupplies',{subtotal:this.Totalsum,year1sum:this.year1sum})
},
save(e,item,totalcost,year1cost){
  ...
  this.items[index].projectSupplies=this.projectSupplies

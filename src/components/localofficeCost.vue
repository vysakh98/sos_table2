...
:items="items">
<template #top>
<div class="top-div"><p class="display-1 white--text">Local Office Costs
<span>
<v-tooltip bottom>
...
<template #item.Local_Office_Costs="{item}">
  <td v-if="editable && editindex==items.indexOf(item)">
     <v-text-field class="tx1" v-model="Local_Office_Costs"></v-text-field>
  </td>
  <td id="td1" v-else>{{item.Local_Office_Costs}}</td>
</template>
...
export default{
	...
	headers:[{text:'',value:'Local_Office_Costs',sortable:false},
...
add(){
  ...
  this.editable=true
  this.items.push({Local_Office_Costs:'', Units:'',Total_No_of_Units:'',Total_Unit_value:'',Total_Total_cost:''
...
deleteitem(item){
   ...
   this.$emit('localofficecost',{subtotal:this.Totalsum,year1sum:this.year1sum})
},
save(e,item,totalcost,year1cost){
  ...
  this.items[index].Local_Office_Costs=this.Local_Office_Costs
